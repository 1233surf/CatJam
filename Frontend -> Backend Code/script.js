  const apiEndpoint = '/api/login'; // Update the URL accordingly
...
function loginUser() {
    ...
    if (matchingUser) {
        // Make a request to the server's /api/login endpoint
        fetch(apiEndpoint, {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            ...
            if (data.success) {
                alert(data.message);
                hideLogin();
                document.getElementById(`page${data.pageNumber}`).classList.add("active");
            } else {
                alert(data.message);
            }
        })
